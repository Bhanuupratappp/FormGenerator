...
const Form = () => {
  ...
  const generateFormId = () => `form_${Date.now()}`;
  const [form, setForm] = useState({
    id: formId || generateFormId(),
    name: "",
    sections: [],
  });
  ...
  useEffect(() => {
    const getFormData = () => {
      ...
      if (storedFormData) {
        const { name, sections } = JSON.parse(storedFormData);
        setFormName(name);
  ...
  const validateForm = () => {
    const newErrors = {};
    if (!formName.trim()) {
      newErrors.formName = "Form name cannot be empty";
    }
                    ...
                    <div className="col-3">
                      <div className="input-group">
                        ...
                        >
                          ...
                          <option value="password">Textarea</option>
